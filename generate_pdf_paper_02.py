...
json_path = r"e:\Sushanth Projects\SURYA\Vaults\SET-SESSION 02 2027 FINAL PAPER.json"
html_path = r"e:\Sushanth Projects\SURYA\Vaults\Printable_SET_Session02.html"
pdf_path = r"e:\Sushanth Projects\SURYA\Vaults\SET_Session02_Paper.pdf"
...
<head>
<meta charset="utf-8">
<title>SET Session 02 - Question Paper</title>
...
<body>
  <div class="header">
    <h1>Surya Entrance Test [SET] 2027 - Session 02</h1>
...
for sec in data.get('sections', []):
    ...
    pos = sec.get("marks", {}).get("pos", sec.get("marks", {}).get("correct", 0))
    neg = sec.get("marks", {}).get("neg", sec.get("marks", {}).get("wrong", 0))
    html_content += f'<div class="section"><div class="sec-header">{sec.get("name", "Section")} ({len(qs)} Questions | +{pos} / -{neg})</div>'
    ...
    for q in qs:
        ...
        if q.get("image"):
            text_content += f'<br><img src="{q.get("image")}" style="max-width:100%; max-height:200px; margin:10px 0; display:block;">'
        html_content += f'<div class="q-content">{text_content}'
